  ...
  return (
    <div className="animate-fadeIn space-y-8">
      ...
      <header className="mb-8">
        ...
        <p className="text-xl text-muted-foreground mt-1">Summary of animal verification status by enclosure.</p>
      </header>

      {zoo.sites.length === 0 ? (
        <p className="text-lg text-muted-foreground">This zoo has no sites configured to report on.</p>
      ) : (
        zoo.sites.map(site => (
          <Card key={site.id} className="shadow-lg">
            <CardHeader>
              <CardTitle className="text-2xl flex items-center text-primary-foreground bg-primary -m-6 p-6 rounded-t-lg">
              ...
              {site.sections.length === 0 ? (
                ...
                site.sections.map(section => (
                  <div key={section.id} className="space-y-3">
                    ...
                    { site.sections.indexOf(section) < site.sections.length -1 && <Separator className="my-6" />}
                  </div>
                ))
              )}
            </CardContent>
